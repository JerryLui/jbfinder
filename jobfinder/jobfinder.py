...
def db_add_jobs():
	...
	for company in dbh.get_companies():  # [id, name, url]
		print(company)
		if not crawl(company[1], company[2]): continue  # Skip if no jobs are found
		for job in crawl(company[1], company[2]):
...
# Get offers
offers = filter_offers(dbh.get_offers(locations), keywords)
